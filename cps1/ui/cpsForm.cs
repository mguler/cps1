...
namespace ui
{
    public partial class cpsForm : Form
    {
        ...
        {
            byte[] bb1 = new byte[0xc00 * 4], bb2 = new byte[0xc00 * 4];
            FileStream fs1 = new FileStream("palette01.dat", FileMode.Create);
            Array.Copy(CPS.gfxram, 0xc000, bb1, 0, 0xc00 * 4);
            bb2 = ByteSwap(bb1);
            fs1.Write(bb2, 0, 0xc00 * 4);
            fs1.Close(); ;
        }
        private void btnColor_Click(object sender, EventArgs e)
        {
            int i;
            int iColor = int.Parse(tbInput.Text);
            StreamWriter sw1 = new StreamWriter("c" + iColor.ToString() + ".txt");
            for (i = 0; i < 15; i++)
            {
                sw1.WriteLine(Palette.entry_color[iColor * 0x10 + i].ToString("x8"));
            }
            sw1.Close();
        }
        private void btnSaveRam_Click(object sender, EventArgs e)
        {
            int i1, idx1;
            byte[][] bb1;
            byte[] bb2;
            ...
            for (i1 = 0; i1 < 8; i1++)
            {
                bb1[i1] = BitConverter.GetBytes(MC68000.m1.D[i1].u32);
                bb1[8 + i1] = BitConverter.GetBytes(MC68000.m1.A[i1].u32);
            }
            for (i1 = 0; i1 < 0x20; i1++)
            {
                bb1[0x10 + i1] = BitConverter.GetBytes(CPS.cps_a_regs[i1]);
                bb1[0x30 + i1] = BitConverter.GetBytes(CPS.cps_b_regs[i1]);
            }
            bb1[0x50] = ByteSwap(Memory.mainram);
            bb1[0x51] = Memory.audioram;
            bb1[0x52] = ByteSwap(CPS.gfxram);
            bb2 = new byte[0x408c0];
            idx1 = 0;
            for (i1 = 0; i1 < 0x53; i1++)
            {
                Array.Copy(bb1[i1], 0, bb2, idx1, bb1[i1].Length);
                idx1 += bb1[i1].Length;
            }
            DumpRam(bb2);
        }
        private void btnLoadRam_Click(object sender, EventArgs e)
        {
            int i1;
            byte[] bb1, bb2, bb3, bb4;
            LoadRam(out bb1, tbInput.Text);
            bb2 = new byte[0x10000];
            bb3 = new byte[0x800];
            bb4 = new byte[0x30000];
            for (i1 = 0; i1 < 8; i1++)
            {
                MC68000.m1.D[i1].u32 = BitConverter.ToUInt32(bb1, 4 * i1);
                MC68000.m1.A[i1].u32 = BitConverter.ToUInt32(bb1, 0x20 + 4 * i1);
            }
            for (i1 = 0; i1 < 0x20; i1++)
            {
                CPS.cps_a_regs[i1] = BitConverter.ToUInt16(bb1, 0x40 + 2 * i1);
                CPS.cps_b_regs[i1] = BitConverter.ToUInt16(bb1, 0x80 + 2 * i1);
            }
            Array.Copy(bb1, 0xc0, bb2, 0, 0x10000);
            Array.Copy(ByteSwap(bb2), Memory.mainram, 0x10000);
            Array.Copy(bb1, 0x100c0, bb3, 0, 0x800);
            Array.Copy(bb3, Memory.audioram, 0x800);
            Array.Copy(bb1, 0x108c0, bb4, 0, 0x30000);
            Array.Copy(ByteSwap(bb4), CPS.gfxram, 0x30000);
        }
        public void DumpRam(byte[] bb1)
        {
            FileStream fs1 = new FileStream("2.dmp", FileMode.Create);
            fs1.Write(bb1, 0, bb1.Length);
            fs1.Close();
        }
        public void LoadRam(out byte[] bb1, string sFile)
        {
            int n1;
                ...
                for (i3 = 0; i3 < cols; i3++)
                {
                    for (i4 = 0; i4 < rows; i4++)
                    {
                        ...
                        for (i1 = 0; i1 < tilewidth; i1++)
                        {
                            ...
                            {
                                iOffset = iCode * 0x40 + gfxset * 4 + (i1 + i2 * 16) / 2;
                                iByte = CPS.gfxrom[iOffset];
                                if (i1 % 2 == 0)
                                {
                                    idx = iColor * 0x10 + iByte % 16;
                                }
                                else if (i1 % 2 == 1)
                                {
                                    idx = iColor * 0x10 + iByte / 16;
                ...
                for (i3 = 0; i3 < cols; i3++)
                {
                    for (i4 = 0; i4 < rows; i4++)
                    {
                        ...
                        for (i1 = 0; i1 < tilewidth; i1++)
                        {
                            for (i2 = 0; i2 < tileheight; i2++)
                            {
                                iOffset = iCode * 0x40 * 2 + (i1 + i2 * 16) / 2;
                                iByte = CPS.gfxrom[iOffset];
                                if (i1 % 2 == 0)
                                {
                                    idx = iColor * 0x10 + iByte % 16;
                ...
                for (i3 = 0; i3 < cols; i3++)
                {
                    for (i4 = 0; i4 < rows; i4++)
                    {
                        ...
                        for (i1 = 0; i1 < tilewidth; i1++)
                        {
                            for (i2 = 0; i2 < tileheight; i2++)
                            {
                                iOffset = iCode * 0x40 * 8 + (i1 + i2 * 16 * 2) / 2;
                                iByte = CPS.gfxrom[iOffset];
                                if (i1 % 2 == 0)
                                {
                                    idx = iColor * 0x10 + iByte % 16;
